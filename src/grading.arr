...
type GradingMetadata = Boolean # whether the test is visible
...
fun grade(graders :: Graders) -> List<{Id; AggregateResult;}> block:
  ...
  for fold(acc :: List<{Id; AggregateResult;}> from [list:], 
    ...
    if meta-dict.get-value(key) block:
...
where:
  grade(
    [list:
      node("guard_1", [list:], lam(): block(invalid) end, false),
      node("guard_2", [list: "guard_1"], lam(): proceed end, false),
      node("test_1", [list: "guard_2"], lam(): done(score(10, 10)) end, true)])
    is
    [list: 
      {"test_1"; 
        aggregate-skipped(
