...
#| wheat (tdelvecc, Sep 7, 2020)
    - generate-input returns empty on negative input;
    - is-valid returns false on invalid companies/candidates.
...
fun is-valid(
  ...
  # WHEAT DIFFERENCE: Checks that the companies/candidates lists are valid
  problem-size = companies.length()
  expected = lists.repeat(problem-size, range(0, problem-size))
  
  (companies.map(lists.sort) == expected) and (candidates.map(lists.sort) == expected) and
