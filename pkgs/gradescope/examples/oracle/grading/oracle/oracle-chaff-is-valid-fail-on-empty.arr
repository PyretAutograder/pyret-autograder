...
#| chaff (tdelvecc, Sep 7, 2020)
    Raises an error when is-valid is passed an empty list.
...
fun is-valid(
  ...
  -> Boolean block:
  # CHAFF DIFFERENCE
  when is-empty(companies):
    raise("empty companies.")
  end
  ...
  is-valid-good(companies, candidates, hires)
end
...
fun oracle(a-matchmaker :: (List<List<Number>>, List<List<Number>> 
  ...
  for lists.all({companies; candidates} from manual-inputs):
    is-valid-good(companies, candidates, a-matchmaker(companies, candidates))
  end and
