...
# Bare (non-method) list/set functions used by some grading implementations;
# `lists`/`sets` are already in scope for prefixed use, so only the non-global
# bare names are imported (importing `lists` itself would shadow and fail).
import list-to-set from sets
import sort-by from lists
...
where:
  # ties: two equally-relevant tweets both appear (any order within the tie
  # class).  The search call is inline in the assertion so an implementation
  # that *raises* on a tie fails this test cleanly (rather than crashing the
  # block).
  block:
    tie-1 = tweet("tie-1", "B", empty)
    tie-2 = tweet("tie-2", "B", empty)
    search(tweet("tie-s", "A", empty), [list: tie-1, tie-2], 0)
      satisfies oracle([list: [list: tie-1, tie-2]], _)
  end
block:
...
block:
  ...
  search-tweet = tweet("search", "B", empty)
...
block:
  ...
  search-tweet = tweet("search", "A B C D", empty)
  
  
  sol = search(search-tweet, [list: tweet-a], 1)
  
  sol is [list: tweet-a]
  
  
  sol2 = search(search-tweet, [list: tweet-a, tweet-b], 1)
  sol2 is empty
end
...
# All tweets are tied is extremely difficult to make...
block:
...
# All tweets are 0 relevances is impossible.
block:
  tweet-a3 = tweet("a3", "A B", empty)
  tweet-a221 = tweet("a221", "B B C", empty)
  tweet-a22 = tweet("a22", "B B C D", [list: tweet-a221])
  ...
  search-tweet = tweet("search", "NOPE", empty)
...
block:
  ...
  tweet-a3 = tweet("a3", dirtify("A B"), empty)
  tweet-a221 = tweet("a221", dirtify("B B C"), empty)
  tweet-a22 = tweet("a22", dirtify("B B C D"), [list: tweet-a221])
  tweet-a21 = tweet("a21", dirtify("A A A A"), empty)
  tweet-a2 = tweet("a2", dirtify("C C D D"), [list: tweet-a21, tweet-a22])
  tweet-a11 = tweet("a11", dirtify("A A B C"), empty)
  tweet-a1 = tweet("a1", dirtify("B C D"), [list: tweet-a11])
  tweet-a = tweet("a", dirtify("A B C D"), [list: tweet-a1, tweet-a2, tweet-a3])

  tweet-b121 = tweet("b121", dirtify("A"), empty)
  tweet-b12 = tweet("b12", dirtify("B B B B"), [list: tweet-b121])
  tweet-b11 = tweet("b11", dirtify("B C D"), empty)
  tweet-b1 = tweet("b1", dirtify("A A B B"), [list: tweet-b11, tweet-b12])
  tweet-b = tweet("b", dirtify("D"), [list: tweet-b1])

  tweet-c = tweet("c", dirtify("A A B C"), empty)

  search-tweet = tweet("search", dirtify("A A B C"), empty)
...
block:
  ...
  search-tweet = tweet("search", funify("A A B C"), empty)
...
block:
  ...
  search-tweet = tweet("search", numify("A A B C"), empty)
