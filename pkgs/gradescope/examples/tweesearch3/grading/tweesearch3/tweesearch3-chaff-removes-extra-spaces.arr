...
# END HEADER
# chaff (tdelvecc): 
#   When tweet is split by space, removes all empty strings.
...
fun compare(doc1 :: String, doc2 :: String) -> Number:
  ...
  # Split by space
  # CHAFF DIFFERENCE: Filters out empty strings after splitting.
  words1 :: List<String> = string-split-all(doc1-prepped, " ").filter(_ <> "")
  words2 :: List<String> = string-split-all(doc2-prepped, " ").filter(_ <> "")
