...
# END HEADER
# chaff (tdelvecc): 
#   The solution is supposed to consider all descendent tweets for
#   what tweets to return. This chaff only considers the direct
#   children and grandchildren of the root tweet; 
#   all tweets that are further down than grandchildren
#   are not kept, even if they meet threshold.
...
fun find-tweets-and-relevance(
  ...
  fun helper(
    ...
    # CHAFF DIFFERENCE: Kills recursion after grandchildren are processed.
    if depth == 3:
      empty
    else:
      ...
      children :: List<Tv-pair<Tweet, Relevance>> =
        ...
        .map(helper(_, some(current-relevance), depth + 1))
