...
# END HEADER
# CHAFF (jdodson4): 
#   Keeps only lowercase characters from tweets.
...
fun compare(doc1 :: String, doc2 :: String) -> Number:
  ...
  # CHAFF DIFFERENCE: string not converted to lowercase when filtering
  # so, it will only keep lowercase characters from the tweet
  prep = lam(doc): string-explode(doc).filter(
