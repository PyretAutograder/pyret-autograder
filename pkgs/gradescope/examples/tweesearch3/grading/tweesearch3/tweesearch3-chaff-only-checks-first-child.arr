...
provide: search end
# END HEADER
# chaff (sli96; edited by tdelvecc): 
#   Only recurses on first child of each tweet.
...
fun find-tweets-and-relevance(
  ...
  fun helper(
    ...
    # CHAFF DIFFERENCEE: Only recurs on first child.
    children :: List<Tv-pair<Tweet, Relevance>> =
      cases (List<Tweet>) current-tweet.children:
        | empty => empty
        | link(f, _) => helper(f, some(current-relevance))
      end
