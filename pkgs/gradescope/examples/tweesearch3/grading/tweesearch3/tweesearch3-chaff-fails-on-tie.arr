...
# END HEADER
# chaff (tdelvecc): 
#   Raises an error when there is a tie of relevance.
...
fun search(
  ...
  # CHAFF DIFFERENCE: Raises an error when two tweets have the same relevance.
  relevances = all-tweets.map({(twt :: Tv-pair<Tweet, Relevance>): twt.value})
  when (relevances.length() <> lists.distinct(relevances).length()):
    raise("Two tweets have the same relevance.")
  end
