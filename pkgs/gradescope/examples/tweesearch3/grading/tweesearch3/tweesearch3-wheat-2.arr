...
# END HEADER
# wheat (tdelvecc): 
#   Flip order of ties from basic wheat.
#    - Raises an error on empty tweets;
#    - Raises an error on invalid thresholds;
#    - Raises an error when given tweet isn't a root tweet;
...
fun compare(doc1 :: String, doc2 :: String) -> Number block:
  ...
  # WHEAT DIFFERENCE: Throw error on empty tweet
  when (doc1 == "") or (doc2 == ""):
    raise("Can't have empty tweet!")
  end
...
fun search(
  ...
  # WHEAT DIFFERENCE: Error on invalid threshold.
  ...
  # WHEAT DIFFERENCE: Reverse order of tweets before sorting to make
  #                   reverse stable sort.
  
  reversed-tweets :: List<Tv-pair<Tweet, Relevance>> =
    all-tweets.reverse()

  # Sort, filter, and extract tweets
  sort-by-key(reversed-tweets, {(t :: Tv-pair<Tweet, Relevance>): t.value})
