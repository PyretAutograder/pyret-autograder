...
provide: search end
# END HEADER
# chaff (sl96; edited by tdelvecc): 
#   Does not use parent relevance in calculations.
...
fun find-tweets-and-relevance(
  ...
  fun helper(
    ...
    # CHAFF DIFFERENCE: Does not take into account parent relevance.
    current-relevance :: Relevance = 
      (0.75 * similarity) + (0.25 * (find-size(current-tweet) / total-tweets))
