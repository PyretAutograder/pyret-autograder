...
# END HEADER
# chaff (tdelvecc): 
#   Fails when there is a tie in relevance.
...
fun compare(doc1 :: String, doc2 :: String) -> Number:
  ...
  alphanumspace = " abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"
...
fun search(
  ...
  # CHAFF DIFFERENCE: Raise an error when two tweets have same relevance,
  #                   but only if they both pass the threshold.
  relevances = unique-tweets.map({(t :: Tv-pair<Tweet, Relevance>): t.value})
  when relevances.length() <> lists.distinct(relevances).length():
    raise("Two tweets have the same threshold!")
  end
