...
#| chaff (mheller6, Sep 3, 2020):
    `campus-tour` should start from the closest Place in the Graph (which might not
    be a Tour stop). This chaff only chooses the closest Tour stop, even if there
    is a closer place.
...
fun campus-tour(tours :: Set<Tour>, start-position :: Point, campus-data :: Graph) -> Path:
  ...
  cases (List) tour-stop-names:
    ...
    | link(loc-name, _) =>
      # CHAFF DIFFERENCE
      places = tour-stop-names.map(campus-data.get(_))
