...
#| wheat (mheller6, Sep 3, 2020): 
    Breaks dijkstra ties reverse-lexicographically.
    Breaks campus-tour ties lexicographically.
    Errors on tour stops that aren't in graph.
    Errors on non-unique tour names
    Search WHEAT DIFFERENCE for source of differences
|#

#| wheat changes (srajesh1, Sep 4, 2021): 
   Error on negative edge weights
   Search WHEAT DIFFERENCE 2021 for source differences
|#
...
fun campus-tour(tours :: Set<Tour>, start-position :: Point, campus-data :: Graph) -> Path block:
  ...
  # WHEAT DIFFERENCE
  tour-names = tours.to-list().map(_.title)
  when lists.distinct(tour-names) <> tour-names:
    raise("non-unique tour names")
  end
  ...
  graph-names = campus-data.names()
  # WHEAT DIFFERENCE
  when any({(stop): not(graph-names.member(stop))}, tour-stop-names):
    raise("tour stop not present in graph")
  end
  ...
  cases (List) tour-stop-names:
    ...
    | link(loc-name, _) =>
      ...
      places = graph-names.to-list().map(campus-data.get(_))
...
fun get-min<T>(h :: Heap<T>%(is-node)) -> HeapPath<T>:
  doc: ```consumes a balanced, non-empty Heap h and produces the smallest
       element in h```
  # WHEAT DIFFERENCE 2021 for source differences
  if h.value.distance-from-start < 0: raise("Negative edge weight") else: h.value end
end
