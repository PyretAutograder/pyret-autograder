...
#| chaff (mheller6, Sep 3, 2020):
    Uses Euclidian instead of Manhattan distance.
...
fun loc-path-comparator(a :: HeapPath, b :: HeapPath) -> Boolean:
  ...
  within-small-num = within(0.0000001)
  if within-small-num(a.distance-from-start, b.distance-from-start):
    a.name < b.name
  else:
...
fun update-estimates-in-heap(
  ...
  distances     = neighbors.map(lam(neighbor-name):
      ...
      # CHAFF DIFFERENCE
      num-sqrt(num-sqr(neighbor.position.x - current-place.position.x) +
        num-sqr(neighbor.position.y - current-place.position.y))
    end)
