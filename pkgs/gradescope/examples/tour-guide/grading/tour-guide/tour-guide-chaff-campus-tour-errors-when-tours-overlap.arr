...
#| chaff (mheller6, Sep 3, 2020):
    `campus-tour` errors when passed overlapping tours (making sure students test the
    behavior that it should not visit the same stop multiple times).
...
fun campus-tour(tours :: Set<Tour>, start-position :: Point, campus-data :: Graph) -> Path block:
  ...
  # CHAFF DIFFERENCE
  tour-stop-names = fold(
      lam(acc :: List<String>, t :: Tour) -> List<String>:
        acc.append(t.stops.to-list())
      end, empty, tours.to-list())
  
  when lists.distinct(tour-stop-names).length() <> tour-stop-names.length():
    raise("overlapping tours")
  end
