...
#| chaff (mheller6, Sep 3, 2020):
    `campus-tour` errors if tour has to visit same stop multiple times.
...
fun campus-tour(tours :: Set<Tour>, start-position :: Point, campus-data :: Graph) -> Path block:
  ...
  tour-path = cases (List) tour-stop-names:
    | empty => empty
    | link(loc-name, _) =>
  ...
  # CHAFF DIFFERENCE
  when lists.distinct(tour-path).length() <> tour-path.length():
    raise("tour revisits nodes")
  end
