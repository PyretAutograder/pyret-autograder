...
#| chaff (mheller6, Sep 3, 2020):
    Dijkstra errors when any dimensions are really large.
...
fun dijkstra(start :: Name, graph :: Graph) -> Set<Path> block:
  ...
  # CHAFF DIFFERENCE
  is-large = lam(x):
    pos = graph.get(x).position
    (num-abs(pos.x) > 1000) or (num-abs(pos.y) > 1000)
  end
  when any(is-large, graph.names().to-list()):
    raise("large dimensions")
  end
