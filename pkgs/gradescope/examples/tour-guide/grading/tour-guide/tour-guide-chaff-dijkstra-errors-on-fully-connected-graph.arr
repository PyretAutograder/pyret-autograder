...
#| chaff (mheller6, Sep 3, 2020):
    Dijkstra errors on fully-connected graph.
...
fun dijkstra(start :: Name, graph :: Graph) -> Set<Path> block:
  ...
  # CHAFF DIFFERENCE
  num-nodes = graph.names().size()
  is-connected = lists.all(lam(cur):
      graph.get(cur).neighbors.size() == (num-nodes - 1)
    end, graph.names().to-list())
  when is-connected:
    raise("fully-connected graph")
  end
