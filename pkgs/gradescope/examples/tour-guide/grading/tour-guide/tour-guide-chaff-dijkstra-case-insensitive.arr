...
#| chaff (mheller6, Sep 3, 2020):
    `dijkstra` incorrectly handles node names in a case-insensitive manner.
...
fun dijkstra(start :: Name, graph :: Graph) -> Set<Path> block:
  ...
  # CHAFF DIFFERENCE
  new-graph-list = graph.names().fold(lam(acc, cur): 
      old-place = graph.get(cur)
      acc.add(place(string-to-lower(old-place.name), 
          old-place.position, 
          old-place.neighbors))
    end, empty-set)
  shadow graph = to-graph(new-graph-list)
  shadow start = string-to-lower(start)
