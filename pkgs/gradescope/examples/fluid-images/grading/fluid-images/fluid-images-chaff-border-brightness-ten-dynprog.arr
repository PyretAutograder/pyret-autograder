...
fun get-bordered-brightnesses(
  ...
  # CHAFF DIFFERENCE: Uses a border brightness of 10 instead of 0.
  border-brightness = 
    if solver == dynamic-programming:
      10
    else:
      0
    end
  
  # The first and last row are all borders.
  ...
  for lists.foldr(result from [list: border-row], pixel-row from pixels):
    # Start with a border on the end, and add pixel by pixel to the left
    for lists.foldr(brightness-row from [list: border-brightness], pixel from pixel-row):
      ...
      ^ link(border-brightness, _) # Add a border to the left of the row
