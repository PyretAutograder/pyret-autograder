...
#| chaff (tdelvecc, Sep 4, 2020): 
    Raises an error when the image is taller than it is wide
   on the first iteration as it is wide in the memoization impl.
...
fun liquify(input :: Image, n :: Number) -> Image block:
  ...
  # CHAFF DIFFERENCE: Raises an error if image is more than
  # twice as tall as it is wide.
  when (solver == memoization) and (input.height > (input.width * 2)):
