...
#| wheat(tdelvecc, Sep 4, 2020): 
    - Returns original image on negative input.
    - Returns the original image if n >= width.
...
fun liquify(input :: Image, n :: Number) -> Image:
  doc: ``` Consumes an image and a number of times to perform the 
       operation. Returns a reduced image object.```
  # WHEAT DIFFERENCE: Returns original image if n < 0.
  # WHEAT DIFFERENCE: Returns original image of n >= input.width
