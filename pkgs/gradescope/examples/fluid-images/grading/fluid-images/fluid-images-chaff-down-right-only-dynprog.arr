...
#| chaff (tdelvecc, Sep 4, 2020): 
    Raises an error if the resulting seam ever goes left instead
    of down and right in the dynamic-programming implementation.
...
fun remove-seam(
    seam :: List<Number>,
    pixels :: List<List<Color>>)
  -> List<List<Color>> block:
  doc: ```Consumes a List of List of Color's and
       removes the pixel in each row corresponding to
       the numbers in seam.```
  # CHAFF DIFFERENCE: Raises an error if any pixel in the seam
  #                   is to the left of the previous pixel.
  when (solver == dynamic-programming) and map2(_ > _, seam, seam.rest).member(true):
    raise("Seam went left.")
  end
