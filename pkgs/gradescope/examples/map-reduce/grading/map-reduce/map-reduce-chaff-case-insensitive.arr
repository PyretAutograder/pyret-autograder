#| chaff (mheller6, Aug 31, 2020):
    Anagrams are grouped together case-insensitively

    Test Suites
      - anagrams-case-sensitive:: Checks that case is sensitive fail
...
fun anagram-map(input :: Tv-pair<String, String>) -> List<Tv-pair<String, String>>:
  ...
  # CHAFF DIFFERENCE
  map({(word): tv(alphabetize(string-tolower(word)), word)}, words)
#|where:
