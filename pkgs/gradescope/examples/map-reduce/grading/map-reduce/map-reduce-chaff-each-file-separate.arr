#| chaff (mheller6, Aug 31, 2020):
    ...
    Assumes that you're supposed to run a map-reduce on each file in the list and then combine them.
    This will fail when the contents of multiple files are related (e.g. if words in one file are
    anagrams of those in another file).

    Test Suites
      - wc-normal-cases:: Checks normal cases
      - anagrams-case-sensitive:: Checks that case is sensitive
      - anagrams-normal-cases:: Checks normal cases
      - anagrams-unique-tags:: Checks that the tags are unique for the same 1, 2, and 3 word anagrams
      - anagrams-duplicate-values:: Checks that there are no duplicate tv values in the list
      - recommend-normal-cases:: Checks normal cases
      - popular-pairs-normal-cases:: Checks normal cases fail
...
fun map-reduce<A, B, M, N, O>(
  ...
  -> List<Tv-pair<M, O>>:
  map({(file): map-reduce-correct([list: file], mapper, reducer)}, input)
    .foldl(lists.append, empty)
  #| where:
  map-reduce(empty, wc-map, wc-reduce) is empty
  map-reduce([list: tv("book1", "a a a"), tv("book2", "a b")], wc-map, 
    wc-reduce) is [list: tv("b", 1), tv("a", 4)]|#
end

fun map-reduce-correct<A, B, M, N, O>(
    input :: List<Tv-pair<A, B>>,
    mapper :: (Tv-pair<A, B> -> List<Tv-pair<M, N>>),
    reducer :: (Tv-pair<M, List<N>> -> Tv-pair<M, O>)) 
  -> List<Tv-pair<M, O>>:
