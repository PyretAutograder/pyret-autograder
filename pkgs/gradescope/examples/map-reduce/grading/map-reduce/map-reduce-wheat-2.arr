#| wheat (mheller6, Aug 31, 2020):
   Verifies the following input properties:
       Each file contains a single input list, with single book descriptions on each line.
       Input lists will always contain at least two books, and they will never contain duplicates.
   Reverses output of:
       anagram-reduce
       anagram-map
       map-reduce
...
# WHEAT DIFFERENCE
fun is-wellformed(book-records :: List<List<String>>) -> Boolean:
  # Each file contains a single input list, with single book descriptions on each line.
  # Input lists will always contain at least two books, and they will never contain duplicates. 
  book-records.map({(record):
      ...
      and (record.length() > 1)
      and (distinct(record).length() == record.length())})
...
fun alphabetize(str :: String) -> String:
  ...
  # WHEAT DIFFERENCE: alphabetizes in reverse order
  list-to-string(string-explode(str).sort().reverse())
  #|where:
  alphabetize("cba") is "cba"
  alphabetize("") is ""
  alphabetize("haha") is "hhaa|#
end
...
fun anagram-map(input :: Tv-pair<String, String>) -> List<Tv-pair<String, String>>:
  ...
  # WHEAT DIFFERENCE
  map({(word): tv(alphabetize(word), word)}, words).reverse()
  #|where:
  anagram-map(tv("words", "star rats tarts")) is 
  [list: tv("ttsra", "tarts"), tv("tsra", "rats"), tv("tsra", "star")]
  anagram-map(tv("words", "")) is empty
  anagram-map(tv("words", "hi")) is [list: tv("ih", "hi")]|#
end


fun anagram-reduce(input :: Tv-pair<String, List<String>>) -> Tv-pair<String, List<String>>:
  # WHEAT DIFFERENCE
  tv(input.tag, distinct(input.value).reverse())
  #|where:
