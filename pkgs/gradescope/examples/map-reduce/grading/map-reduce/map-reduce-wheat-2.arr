...
#| wheat (mheller6, Aug 31, 2020):
    Reverses output of map-reduce, anagram-map, anagram-reduce.
    Validates input for recommend and popular-pairs.
...
fun map-reduce<A, B, M, N, O>(
  ...
  out = for fold(answer from empty, grp :: List<Tv-pair<M, N>> from grouped):
    cases (List) grp:
  ...
  # WHEAT DIFFERENCE: reverses output
  out.reverse()
end

fun anagram-map(input :: Tv-pair<String, String>) -> List<Tv-pair<String, String>>:
  # WHEAT DIFFERENCE: reverses output and alphabetizes in reverse order
  fun alphabetize-rev(str :: String) -> String:
    list-to-string(string-explode(str).sort().reverse())
  end
  map({(word): tv(alphabetize-rev(word), word)}, parse(input.value, " ")).reverse()
end

fun anagram-reduce(input :: Tv-pair<String, List<String>>) -> Tv-pair<String, List<String>>:
  # WHEAT DIFFERENCE: reverses the deduplicated list
  tv(input.tag, lists.distinct(input.value).reverse())
end

fun recommend(title :: String, book-records :: List<Tv-pair<String, List<String>>>)
  -> Tv-pair<Number, List<String>> block:
  # WHEAT DIFFERENCE: validates input
  fun is-wellformed(records :: List<List<String>>) -> Boolean:
    records.map({(record):
        (record.map(string-length).filter(_ > 0).length() == record.length())
        and (record.length() > 1)
        and (lists.distinct(record).length() == record.length())})
      .foldl({(a, b): a and b}, true)
  end
  when not(is-wellformed(book-records.map(_.value))):
...
fun popular-pairs(book-records :: List<Tv-pair<String, List<String>>>)
  -> Tv-pair<Number, List<BookPair>> block:
  # WHEAT DIFFERENCE: validates input
  fun is-wellformed(records :: List<List<String>>) -> Boolean:
    records.map({(record):
        (record.map(string-length).filter(_ > 0).length() == record.length())
