#| chaff (mheller6, Aug 31, 2020):
    Creates duplicate pairs: for any two books A and B, creates a pair A+B and one B+A
   
    All tests involving popular-pairs should fail with this solution:
      - popular-pairs-normal-cases:: Checks normal cases
...
fun combos(lst :: List<String>) -> List<BookPair>:
  ...
  cases(List) lst:
    ...
    | link(first, rest) => 
      # CHAFF DIFFERENCE
      map({(elt): [list: pair(first, elt), pair(elt, first)]}, rest)
