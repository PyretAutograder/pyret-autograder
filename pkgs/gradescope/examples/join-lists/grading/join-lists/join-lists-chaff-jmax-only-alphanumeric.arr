...
# END HEADER
#| chaff 
   original (mheller6, Sep 2, 2020): 
   (jchen345, Sep 7, 2021)
    Raises an error on non-alphanumeric input to `j-max` to emulate
    not using the passed in comparator.
...
fun j-max<A>(jl :: JoinList<A>%(is-non-empty-jl), 
  ...
  block:
    # CHAFF DIFFERENCE
    when (j-length(jl) > 0) and not(is-string(j-first(jl)) or is-number(j-first(jl))):
  ...
  j-max([list: person("a")], lam(x, y): true end) is [list: person("a")]|#
end
