...
fun mst-kruskal(graph :: Graph) -> List<Edge> block:
  # WHEAT DIFFERENCE
  when not(connected(graph)):
    raise("unconnected")
  end
  ...
  for fold(result :: Graph from empty, e :: Edge from sorted-edges):
    ...
    if is-in-same-set(start, dest) block:
      ...
      # WHEAT DIFFERENCE
      shadow e = edge(e.b, e.a, e.weight)
...
fun mst-cmp(g :: Graph, mst1 :: Graph, mst2 :: Graph) -> Boolean block:
  doc: "Checks if two mst algorithms are valid on a graph"

  # WHEAT DIFFERENCE
  when not(connected(g)):
    raise("unconnected")
  end
