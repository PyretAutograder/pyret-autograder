...
fun generate-st(max :: Number, left :: Number, nodes :: List<String>, edges :: List<Edge>)
  ...
  if left == 0:
    ...
    dest = "n_" + num-to-string(left)
    new-edge = generate-edge(src, dest)
    generate-st(max, left - 1, link(dest, nodes), link(new-edge, edges))
  end
end
...
fun generate-input(n :: Number) -> Graph:
  ...
  if n < 2:
    ...
    # CHAFF DIFFERENCE
    # Breaks first edge in tree, disconnecting the graph.
    # Introduces self-loop so graph produced contains same number of nodes.
    cases (List) edges:
      | empty => empty
      | link(f, r) => link(edge(f.b, f.b, f.weight), link(edge(f.a, f.a, f.weight), r))
    end
  end
end

fun generate-input-real(n :: Number) -> Graph:
  doc: ```Given a number n, generates a dense graph with potential multiple edges and n nodes```
  if (n >= 0) and (n < 2):
    empty
  else:
...
fun sort-o-cle(alg1 :: (Graph -> Graph), alg2 :: (Graph -> Graph)) -> Boolean:
  ...
  alg-cmp(pyramid) and
  # Random tests
  alg-cmp(generate-input-real(10)) and
  alg-cmp(generate-input-real(25)) and
  alg-cmp(generate-input-real(40))
end
