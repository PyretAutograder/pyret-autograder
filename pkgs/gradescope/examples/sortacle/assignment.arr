...
# ---- helper sorters for the oracle tests ----
...
check "generate-input":
  # n = 0 produces the empty list (inline, so a generate-input that *raises*
  # on zero fails this assertion cleanly rather than crashing the block)
  generate-input(0) is empty
  generate-input(0).length() is 0
  block:
    # generate-input-makes-people:: generate input should produce a list of        peo
  ...
  block:
    # generate-input-correct-lengths:: generate input        produces lists of correct
    for map(num-people from range(0, 11)):
      people = generate-input(num-people)
...
check "is-valid":
  block:
    # is-valid-trivial-cases:: is-valid should identify trivial case       lists```:
    bob = person('Bob', 5)
  ...
  block:
    # is-valid-different-sizes:: is-valid should identify        lists with different 
    bob = person('Bob', 5)
    frank = person('Frank', 10)
  ...
  block:
    # is-valid-different-people:: is-valid should identify        lists with different
    bob = person('Bob', 5)
    frank = person('Frank', 10)
    joe = person('Joe', 5)
    joseph = person('Joe', 7)
  ...
  block:
    # is-valid-different-repetition:: is-valid should identify lists        with diffe
  ...
  block:
    # is-valid-correctly-sorted:: is-valid should identify lists that are        sorte
  ...
  block:
    # is-valid-incorrectly-sorted:: is-valid should identify lists that        are sor
    alvin = person('Alvin', 4)
    bob = person('Bob', 5)
    joe = person('Joe', 5)
    joseph = person('Joe', 7)
...
check "oracle":
  block:
    # oracle-correct-sorts:: oracle outputs True for correct sorting        algorithms
  ...
  block:
    # oracle-simple-bad-sorters:: simpler bad sorting algorithms that oracle        sh
  ...
  block:
    # oracle-medium-bad-sorters:: slightly trickier to catch.```:
  ...
  block:
    # oracle-edge-case-bad-sorters:: more complex bad sorting algorithms":
  ...
  block:
    # oracle-reverse-bad-sorter:: specific, possibly hard-to-catch bad       sorting a
