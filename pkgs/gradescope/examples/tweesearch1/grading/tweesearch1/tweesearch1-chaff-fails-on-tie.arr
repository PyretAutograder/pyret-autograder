...
# END HEADER
# chaff (tdelvecc): 
#   Raises an error when two tweets that pass the threshold
...
fun search(
  ...
  output = sort-by-key(alot, relevance(_, search-tweet))
  ...
  relevances = output.map({(t): relevance(t, search-tweet)})
  when relevances.length() <> sets.list-to-set(relevances).size():
