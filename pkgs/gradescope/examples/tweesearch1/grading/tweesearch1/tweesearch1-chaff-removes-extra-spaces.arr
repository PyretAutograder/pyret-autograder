...
# END HEADER
# chaff (gglass):
#   Removes additional spaces in consecutive sequence of space.
#   "a  b" is treated as [list: "a", "b"] instead of [list: "a", "", "b"]
...
fun compare(doc1 :: String, doc2 :: String) -> Number:
  ...
  # Split by space
  words1-pre :: List<String> = string-split-all(doc1-prepped, " ")
  words2-pre :: List<String> = string-split-all(doc2-prepped, " ")
  
  # CHAFF DIFFERENCE: Strips out empty strings before comparing
  words1 = words1-pre.filter(lam(s): not(string-equal(s, "")) end)
  words2 = words2-pre.filter(lam(s): not(string-equal(s, "")) end)
