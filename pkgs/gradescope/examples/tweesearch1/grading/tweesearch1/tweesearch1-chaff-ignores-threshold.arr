...
# END HEADER
# chaff (tdelvecc): 
#   Includes all tweets if threshold is anything less than 1.
...
fun search(
  ...
  # CHAFF DIFFERENCE: Does not filter by threshold if threshold < 1!
    .filter({(t): (threshold < 1) or (relevance(t, search-tweet) >= threshold)})
end
